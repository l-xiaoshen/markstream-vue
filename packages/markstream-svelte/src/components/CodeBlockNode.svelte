<script lang="ts">
  ...
  import { sanitizeClassToken } from "./shared/node-helpers";
  ...
  function resolveRecoverableFallbackLanguage(error: unknown) {
    ...
    const missingLanguage = message.match(
      /Language `([^`]+)` is not included/,
    )?.[1];
    return missingLanguage &&
      isLikelyIncompleteLanguageIdentifier(missingLanguage)
      ? missingLanguage
      : "";
  }
  ...
  function isStreamingLanguagePrefix(lang: string) {
    const token =
      lang.trim().split(/\s+/)[0]?.split(":")[0]?.toLowerCase() || "";
    return (
      token.length >= 3 &&
      streamingLanguageTokens.some(
  ...
  let loadingSettledRefreshTimer: ReturnType<typeof setTimeout> | null =
    $state(null);
  ...
  let rawLanguage = $derived((node.language || "").trim());
  let canonicalLanguage = $derived(normalizeLanguageIdentifier(rawLanguage));
  let monacoLanguage = $derived(
    resolveMonacoLanguageId(canonicalLanguage || rawLanguage || "plaintext"),
  );
  let code = $derived(getResolvedCode(node));
  let diff = $derived(Boolean(node.diff));
  let originalCode = $derived(node.originalCode || "");
  let updatedCode = $derived(node.updatedCode || "");
  ...
  let mergedMonacoOptions = $derived({
    ...(resolvedThemes?.monacoOptions || {}),
    ...(monacoOptions || {}),
  });
  let resolvedMonacoOptions = $derived(buildResolvedMonacoOptions());
  let requestedTheme = $derived(
    getThemeName(
      resolvedIsDark
  ...
  let defaultCodeFontSize = $derived(
    Number(mergedMonacoOptions.fontSize) || 13,
  );
  let minWidthValue = $derived(
    resolveCssSize(minWidth ?? resolvedThemes?.minWidth),
  );
  let maxWidthValue = $derived(
    resolveCssSize(maxWidth ?? resolvedThemes?.maxWidth),
  );
  ...
  let languageIcon = $derived(
    getLanguageIcon(canonicalLanguage || rawLanguage || "plain"),
  );
  let displayLanguage = $derived(
    languageMap[canonicalLanguage] ||
      (rawLanguage ? rawLanguage.toUpperCase() : languageMap[""]),
  );
  let isPreviewable = $derived(
    isShowPreview !== false &&
      (canonicalLanguage === "html" || canonicalLanguage === "svg"),
  );
  let previewTitle = $derived(
    canonicalLanguage === "svg"
  ...
  let shouldDeferStreamingLanguage = $derived(
  ...
  let preLanguageClass = $derived(
    sanitizeClassToken(rawLanguage || monacoLanguage),
  );
  let showPreWhileMonacoLoads = $derived(
    !useFallback &&
      !shouldDelayEditor &&
      !shouldDeferStreamingLanguage &&
      !editorReady,
  );
  let showPreFallback = $derived(
    useFallback ||
      shouldDelayEditor ||
      shouldDeferStreamingLanguage ||
      showPreWhileMonacoLoads,
  );
  let settledRefreshSignature = $derived(
    diff
  ...
  $effect(() => {
    if (
      useFallback &&
      fallbackLanguage &&
      rawLanguage !== fallbackLanguage &&
      isLikelyIncompleteLanguageIdentifier(fallbackLanguage)
    ) {
      useFallback = false;
      fallbackLanguage = "";
    }
  ...
  onDestroy(() => {
    ...
    if (languageRetryTimer) clearTimeout(languageRetryTimer);
    if (loadingSettledRefreshTimer) clearTimeout(loadingSettledRefreshTimer);
  ...
  function buildThemeList() {
    ...
    for (const item of resolvedThemes?.themes || []) add(item);
    for (const item of themes || []) add(item);
    return list;
  }
  ...
  function buildResolvedMonacoOptions() {
    ...
    const diffUnchangedRegionStyle =
      raw.diffUnchangedRegionStyle ?? "line-info";
    const diffDefaults = {
      ...
      useInlineViewWhenSpaceIsLimited:
        raw.useInlineViewWhenSpaceIsLimited ?? false,
    ...
    return {
      ...
      ...raw,
      experimental,
      ...(activeHideUnchangedRegions === undefined
        ? {}
        : { hideUnchangedRegions: activeHideUnchangedRegions }),
      diffHideUnchangedRegions: activeDiffHideUnchangedRegions,
      ...finalOptions,
    };
  }
  ...
  function syncRuntimeMonacoOptions() {
    ...
    for (const key of Object.keys(runtimeMonacoOptions)) {
      if (!(key in nextOptions)) delete runtimeMonacoOptions[key];
    }
  ...
  async function ensureMonaco() {
    if (helpers || useFallback || typeof window === "undefined") return;
    if (ensureMonacoPromise) return ensureMonacoPromise;
  ...
  function queueThemeSync() {
    ...
    void Promise.resolve(helpers.setTheme?.(requestedTheme)).catch((error) => {
      if (typeof console !== "undefined")
        console.warn(
          "[markstream-svelte] Failed to apply Monaco theme:",
          error,
        );
    });
  }
  ...
  async function syncEditor() {
    ...
    await ensureMonaco();
    if (!mounted || useFallback || !helpers) return;
    ...
    try {
      if (diff && typeof helpers.updateDiff === "function")
        await Promise.resolve(
          helpers.updateDiff(originalCode, updatedCode || code, monacoLanguage),
        );
  ...
  async function recreateEditor(kind: "single" | "diff") {
    ...
    createEditorPromise = (async () => {
      try {
        cleanupEditor(false);
        if (!mounted || !editorHost || lifecycleId !== creationId) return;
  ...
  function refreshEditorAfterLoadingSettled() {
    if (loadingSettledRefreshPromise) return loadingSettledRefreshPromise;
  ...
  function cleanupEditor(disposeHelpers = true) {
    ...
    try {
      if (disposeHelpers) helpers?.cleanupEditor?.();
      else (helpers?.safeClean || helpers?.cleanupEditor)?.();
    } catch {}
    editorKind = null;
    editorReady = false;
    lastLayoutWidth = null;
  ...
  function getMaxHeightValue() {
    ...
    if (raw === "none" || raw == null) return Number.POSITIVE_INFINITY;
    const value =
  ...
  function scheduleEditorHeightSync() {
    if (typeof window === "undefined" || !editorHost || !editorReady) return;
    if (heightSyncRaf != null) return;
  ...
  function cancelEditorHeightSync() {
    if (heightSyncRaf == null || typeof window === "undefined") return;
  ...
  function scheduleEditorTokenization(delay = 140, refreshModelValue = false) {
    ...
    tokenizeShouldRefreshModelValue =
      tokenizeShouldRefreshModelValue || refreshModelValue;
    if (tokenizeTimer) clearTimeout(tokenizeTimer);
    tokenizeTimer = setTimeout(() => {
      tokenizeTimer = null;
      if (tokenizeRaf != null) return;
  ...
  function cancelEditorTokenization() {
    ...
    tokenizeShouldRefreshModelValue = false;
    if (tokenizeRaf == null || typeof window === "undefined") return;
  ...
  function forceTokenizeEditorModel(refreshModelValue = false) {
    try {
      ...
      if (
        typeof forceTokenization !== "function" ||
  ...
  function bindEditorHeightSync() {
    ...
    const bind = (
      ...
      try {
        ...
        const disposable = subscribe.call(source, () =>
          scheduleEditorHeightSync(),
        );
        if (disposable) heightSyncDisposables.push(disposable);
      } catch {}
    };
  ...
  function syncEditorHostHeight() {
    if (!editorHost || !helpers || !editorReady || collapsed) return;
    ...
    const cappedHeight =
      expanded || !Number.isFinite(maxHeight)
    ...
    editorHost.style.maxHeight =
      expanded || !Number.isFinite(maxHeight)
        ? "none"
        : `${Math.ceil(maxHeight)}px`;
    editorHost.style.overflow = diff
      ? "hidden"
  ...
  function getEditorHostMinHeight() {
    if (!editorHost || typeof window === "undefined") return 0;
    const values = [
  ...
  function computeEditorContentHeight() {
    try {
      if (diff) {
        ...
        if (height > 0) return Math.ceil(height + 1);
      }
      const editor = helpers?.getEditorView?.();
  ...
  function measureRenderedDiffHeight(container: HTMLElement) {
    ...
    try {
      ...
      if (bottom > 0) return Math.ceil(bottom + 1);

      const diffRoot = container.querySelector<HTMLElement>(
        ".monaco-diff-editor",
      );
  ...
  function decreaseFont() {
    if (!enableFontSizeControl) return;
    ...
    {#if showHeader}
      <div class="code-block-header">
        <div class="code-block-header__meta">
          <span class="code-block-language-icon" aria-hidden="true"
            >{@html languageIcon}</span
          >
          <span class="code-block-header__label"
            >{diff ? `Diff / ${displayLanguage}` : displayLanguage}</span
          >
        </div>
        <div class="code-block-header__actions">
          ...
          {#if showFontSizeButtons && enableFontSizeControl}
            <button
              ...
              onfocus={(event) =>
                showButtonTooltip(event, t("common.decrease") || "Decrease")}
              onmouseleave={() => hideTooltip()}
              onmouseenter={(event) =>
                showButtonTooltip(event, t("common.decrease") || "Decrease")}
            >
            ...
            <button
              ...
              onclick={resetFont}
              onfocus={(event) =>
                showButtonTooltip(event, t("common.reset") || "Reset")}
              onmouseleave={() => hideTooltip()}
              onmouseenter={(event) =>
                showButtonTooltip(event, t("common.reset") || "Reset")}
            >
            ...
            <button
              ...
              onfocus={(event) =>
                showButtonTooltip(event, t("common.increase") || "Increase")}
              onmouseleave={() => hideTooltip()}
              onmouseenter={(event) =>
                showButtonTooltip(event, t("common.increase") || "Increase")}
            >
          ...
          {#if isPreviewable && showPreviewButton}
            <button
              ...
              onfocus={(event) =>
                showButtonTooltip(event, t("common.preview") || "Preview")}
              onmouseleave={() => hideTooltip()}
              onmouseenter={(event) =>
                showButtonTooltip(event, t("common.preview") || "Preview")}
            >
            ...
            >
              <svg viewBox="0 0 24 24" aria-hidden="true"
                ><path
                  ...
                  stroke-width="2"
                  d={expanded
                    ? "m14 10l7-7m-1 7h-6V4M3 21l7-7m-6 0h6v6"
                    : "M15 3h6v6m0-6l-7 7M3 21l7-7m-1 7H3v-6"}
                /></svg
              >
            </button>
          {/if}
      ...
      <div class:code-block-body--expanded={expanded} class="code-block-body">
        {#if !shouldDelayEditor}
          <div
            ...
            class:is-hidden={showPreFallback}
            class="code-editor-container"
          ></div>
        {/if}
        {#if showPreFallback}
          <pre class="code-pre-fallback"><code
